...
#socket通信
    ...
    #the robot's init position is base on odom coordinate 
    self.x = 0.0
    self.y = 0.0
    self.th = 0.0
...
def callback(self, move_cmd):
    ...
    #三个轮子的角速度 rad/s
    wtheta1 = (sina * vy - cosa * vx + w * L) / r     #Va
    ...
    asp[4]=(k1>>8)&0xFF#vlint1   Va
    asp[5]=k1 & 0xFF

    asp[6]=(k2>>8)&0xFF  #vrint2  Vb
    asp[7]=k2 & 0xFF

    asp[8]=(k3>>8)&0xFF#vlint3   Vc
    asp[9]=k3 & 0xFF
...
def talker(self):
                ...
                if buf:
                    ...
                    wrad1 = math.pi * 2 * Encoder1_diff / Ticks #Encoder1_diff点数对应的弧度数  how many round /one round with ticks
                    wrad2 = math.pi * 2 * Encoder2_diff / Ticks #Encoder1_diff点数对应的弧度数
                    wrad3 = math.pi * 2 * Encoder3_diff / Ticks #Encoder1_diff点数对应的弧度数
                    wtheta1 = wrad1 / dt #角速度,rad/s    A wheel
                    wtheta2 = wrad2 / dt #角速度,rad/s    B wheel
                    wtheta3 = wrad3 / dt #角速度,rad/s    C wheel
